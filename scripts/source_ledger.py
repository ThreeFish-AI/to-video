...
用法（零第三方依赖）。注意 `--project` 定义在顶层 parser，须写在子命令**之前**：
    uv run --no-project $T/scripts/source_ledger.py --project $P list
    uv run --no-project $T/scripts/source_ledger.py --project $P verify
    uv run --no-project $T/scripts/source_ledger.py --project $P fetch \\
        --name s01-repo --kind repo --via "pinned commit" \\
        --url https://raw.githubusercontent.com/o/r/<sha>/s01/README.zh.md
    # sync/audit 消费系列信源地图（多章批量取证，规格：references/01 §多章批量取证）：
    uv run --no-project $T/scripts/source_ledger.py --project $P sync \\
        --map $W/source-map/<series>.toml --episode 2 [--dry-run] [--refetch]
    uv run --no-project $T/scripts/source_ledger.py --project $P audit \\
...
def render_ledger(entries: dict) -> str:
    head = (
        "# 非论文信源清单（可复现取证链，工具：$T/scripts/source_ledger.py）\n"
        "#\n"
...
# ------------------------------------------------------------------ 多章批量取证
# 规格出处：references/01-source-extraction.md §多章批量取证（B 型信源）。
