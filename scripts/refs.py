...
用法（零第三方依赖；rebuild 才需要 soundfile/numpy）：
    uv run --no-project $T/scripts/refs.py list
    uv run --no-project $T/scripts/refs.py verify [--name me-bright]
    uv run --no-project --with soundfile --with numpy $T/scripts/refs.py \
