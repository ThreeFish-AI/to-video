...
主题对比度（--check-theme，零依赖、不需要视频）：
    ...
    theme.bg；概念色 < 4.5:1 → FAIL（此前只能肉眼估，见 references/08 清单）。

抽帧计划（--stills-plan，零依赖、不需要视频）：
    TTS 长跑中途的分幕复检排期器（references/09 ★节步骤 2–3 的机械化）：读部分
...
用法：uv run --no-project [--with pillow --with numpy] $T/scripts/qa_frames.py \
          --project $P [--lang zh|en] <video.mp4> [--scene P2|--last-n 6|句id…] [--check]
     uv run --no-project $T/scripts/qa_frames.py --project $P --check-theme
     uv run --no-project $T/scripts/qa_frames.py --project $P --stills-plan [--chars-per-sec 5]
...
def check_theme(root: Path, msgs: list[str]) -> None:
    ...
    for key, val in colors.items():
        ...
        if ratio < CONTRAST_MIN:
            msgs.append(
                f"FAIL {key} {val} 对 bg 对比度 {ratio:.2f}:1 < {CONTRAST_MIN}（references/08 视觉契约：概念色须 ≥4.5:1）"
            )
