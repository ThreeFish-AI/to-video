...
- 输出：`$V/<名字>.wav`（`$V` 见 references/PIPELINE.md 路径变量约定）—— 16-bit PCM 单声道，保留原始采样率
...
用法：uv run --no-project --with soundfile $T/scripts/prepare_ref.py \
...
def main() -> int:
    ...
    parser.add_argument(
        "--out",
        default=None,
        help="输出路径（默认写入工作区 $V/<源文件名>.wav，即 $W/voices/）",
    )
