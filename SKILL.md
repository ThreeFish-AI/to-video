...
本 Skill 是**路由层**：内容 SSOT 在 `references/NN-*.md`（阶段规格，文件号 = 阶段序号），工具 SSOT 在 `scripts/`，全部阶段的唯一机器可读声明是 [references/stages.toml](references/stages.toml)；此处只给路由、指针与不变量，不复制正文（防第二事实源）。
...
| 润色成稿（语句断续、像 AI 写的） | 按 ⑤ 规格四层 pass 原地改稿，事实与句 id 冻结；改动句回 ④A 复核 → `build` → `check` | 速查表 ⑤ |
...
# 3) ①–⑥ 内容层：$P/research/ 取证 → planning.md → narration.md →（④ 校验 → ⑤ 成文优化）→ storyboard.md；
...
| ⑤ 成文优化 | 四稿按结构→衔接→句子→词句四层 pass 改成人写模样；只改表达不改事实 | [05](references/05-prose-refinement.md) | —（authored；改后 `build` + `check`） | 成文评审 REWRITE=0 且改动句复核 RISKY=0 |
...
| ⑨ 草渲 + 抽帧 QA | 半分辨率快速迭代（`--beat-heads` 补入场瞬态盲区） | [09](references/09-render-qa.md) | `render` + `qa` | qa --check 自动体检零 FAIL（含尾幕渐黑必查） |
...
| 文件 | 何时读 |
|---|---|
| 阶段规格 `references/NN-*.md` | 进入该阶段时（入口见速查表） |
