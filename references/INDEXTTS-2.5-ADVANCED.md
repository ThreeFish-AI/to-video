...
`emo_audio_prompt` 与 `emo_vector` 被本服务显式拒绝同传。理由有二（第 1 条为 2026-09-25
核源码 + 字节级实测勘误）：**(1)** 上游 `infer_v2_5.py:582-585` 只要给了 `emo_vector` 就把
`emo_audio_prompt` 置 None——**音频被整个丢弃**（向量+音频同传的输出与纯向量逐字节一致；
旧文档「音频仍会以 (1−Σw) 权重混进最终 emovec」的表述错误，`(1−Σw)` 份额实际来自**本人**
`spk_audio_prompt` 的情感编码）；**(2)** `emo_alpha` 被消费两次——先在 `:605-608` 缩放向量，又在 `:763`
