...
[take]                               # take 验收的块重掷（写稿时不填）：该句所在块种子 +N
p1-16 = 1                            # 1–999；只重录这一块，定稿值留档即可复现
```
...
3. 表演标点（`…` 拖停顿、`！` 抛高点、关键短语前加逗号顿一下，`——` 同属可换的停顿标点）只许改标点，
   `build` 会校验「去标点后与正文全等」（数字里的 `3.5`、`1,200`、`10:30` 算正文字符；往数字串里插标点、删掉两数之间的标点同样拒），改字必须回 narration.md；换标点也可能踩读法陷阱（两数之间写 `—` 会读成「减」），`check` 的读法陷阱门对 say 文本同样执法；
4. 发音标注句的 `say` 必须原样携带 `<字|读音>`（build 逐个校验：删掉任一处、改读音或往标注内部插标点均拒）。

`[take]` 不属于导演规则，是合成后的 take 验收工具（[VOICE-CLONING §5.4](VOICE-CLONING.md)）：
块是重掷单位，同一块内只许一条 `[take]`（`tts.py` 在 `--plan` 即报错）。

台本只认 `[block]` / `[say]` / `[take]` 三张表，`[block.<句id>]` 只认 `emo` / `alpha`——
拼错的表名或键名（`[blocks]`、`alhpa`）`build` 直接 FAIL，不会静默忽略。
...
初稿过 ④ 后交 [⑤ 成文优化](./05-prose-refinement.md)：本节纪律管「写对」，⑤ 管「写顺、像人写」——beat 分段、段间衔接、为耳朵写的句子节奏都在那里。
