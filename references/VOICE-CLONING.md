...
uv run --frozen --with fastapi --with uvicorn --with soundfile --with numpy --with lameenc \
    python $T/scripts/tts_server.py \
...
服务端按需拉起、用完即关，**不要保持常驻**：常驻占用 10 GiB 量级 MPS 显存，且长跑累积服务态（健康端点不反映的劣化，见 §七「合成全 500 而 health 假绿」）；重启即最干净的状态复位，冷启动仅 ~1–2 分钟。制作 Agent 的执行纪律（含同机多 Agent 互斥判据）见 [references/06-tts-voice.md](./06-tts-voice.md)「服务生命周期」；要点：
...
uv run --no-project --with soundfile --with numpy \
    $T/scripts/prepare_ref.py ~/Documents/dify/me-1.mp3 \
...
```bash
uv run --no-project --with soundfile --with numpy $T/scripts/prospect_ref.py \
...
```bash
uv run --no-project --with soundfile --with numpy \
    $T/scripts/prospect_ref.py --accept <候选1.wav> [<候选2.wav> …]
...
1. **`me-1` @106s 是目前所有已勘探选段里韵律动态最大的一段**：`IQR_rel` 比成片在用的 `me-bright` 段高 **+18%**，静音占比高 0.08——即「起伏更大且停顿更多」，正是抑扬顿挫的两个维度。且它与 `me-bright`（@0.36s）、`me-1.wav`（@180s）同源不同窗，无重叠。复现：`uv run --no-project --with soundfile --with numpy $T/scripts/prepare_ref.py ~/Documents/dify/me-1.mp3 --start 106 --duration 12 --out $V/me-1-106.wav`（sha1 `3c109eff6aa8`）。
...
|---|---|---|---|---|---|---|
| **story 段落演绎** | **默认推荐位**（2026-09-25 第三轮试听定档 #07）：合成单位＝故事块，见 §4.5 | happy=1/3, surprised=2/3 | **0.28** | **0.28** | 1.0 | 1 |
| **sunny 明快阳光** | 日常/批量备选（2026-08-19 试听定档） | happy=.95, surprised=.02, calm=.03 | **0.35** | **0.35** | 0.95 | 1 |
| **sunny-steady 明快稳健** | 旧成片定稿档（14 个存量集在用） | 同 sunny | 0.35 | 0.35 | 0.95 | **3** |
| neutral | 中性（逐句合成缺省） | 不注入情感，纯克隆参考音色 | — | 0 | 1.0 | 1 |
...
> 并成门（见 [references/03-narration.md](./03-narration.md) 读法纪律）。
...
**story 档把合成单位从「句」升为「故事块」**：同幕连续句拼一次 `/synthesize`（上游对 ≤118 token 的文本合并为单段连续生成），句间是模型自带的自然停顿；服务端把块音频按句界切回逐句 mp3（Remotion 契约不变）。定档依据（2026-09-25 第三轮试听 #07，dream-rsi P0）：「朗读感」的根因不是情绪强度，而是**逐句独立合成 + 全片单一情绪 + 恒定句距**的结构；段落合成 + 块情绪 + 表演标点后，弧线转正、音色门（WavLM）0.954/0.938 为全部候选中最稳。

- **块划分**：幕界 + 台本块起点为硬边界；其余自动划分（≤3 句、≤90 字，取可行的最少块数）；台本块超限被拆开时，子块沿用块首情绪。块文本按原稿标点拼接，`，：、——` 结尾原样保留（续接语气），无标点结尾才补 `。`（收引号/括号不算末字，`！”` 按 `！` 判定）。EN 未验证，story 档下自动回退逐句。
...
uv run --no-project --with soundfile --with numpy \
    $T/scripts/prepare_ref.py ~/Documents/dify/me-1.mp3 --start 0.36 --duration 12 \
