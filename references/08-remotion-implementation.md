# Stage ⑧ Remotion 场景实现（skill 规格 · 08）
...
uv run --no-project $T/scripts/verify_skeleton.py --strict  # 有未登记漂移即失败
```
...
- **chrome 层（机械排版/标注）已随骨架播种**：`Panel` / `Footnote` / `SceneTag` /
  ...
  [motifs.tsx](../assets/video-skeleton/video/src/components/motifs.tsx)（seeded 档，
...
机制 SSOT 是 frozen [i18n.tsx](../assets/video-skeleton/video/src/i18n.tsx)（`LangProvider` 由 `Main.tsx` 挂载，`lang` 经 `remotion render --props '{"lang":"en"}'` 注入、缺省 zh）：
...
规格 SSOT 在模板 [ChapterProgress.tsx](../assets/video-skeleton/video/src/components/ChapterProgress.tsx)，
此处只锚定不可漂移的设计事实：
...
- **数据面**：段边界/占比来自 `computeTimeline` 的 `scenes`（manifest 实测时长驱动，TTS 重跑自动重定时）；
  ...
  派生（见 references/03「幕标题即章节标签」）。chapters 为空（首次 build 前）组件自渲染 null。
