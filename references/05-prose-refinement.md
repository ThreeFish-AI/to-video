...
1. **改前留底**：工作区在 git 下即以 `git diff` 定位改动；否则改前把四稿复制到 `$P/.temp/prose-before/`，事后清理。
2. **改动表**：每处改动一行——`句 id 或位置 | 规则编号（如 L2-1、四-9、Z5）| 一句理由`。规则编号引用本规格的节号与条号。
3. **改动句复核**：只对改动过的句子重跑 [④A 真实性校验](./04-verification.md) 的四级判定。任何一句判 RISKY 即回退原句；REWRITE 则按 ④ 修正。未改动的句子不重复核查。
4. **重跑机器门**：`uv run --no-project $T/scripts/pipeline.py --project $P build`，然后 `check`（双语集会同时点名 en 失配句，按第二节第 6 条处理）。
5. **版本标注**：逐字稿头部改为 `（vN，已过双重校验与成文优化）`，版本号加一。
...
- [ ] 逐句朗读通过，没有念不顺的句子；
- [ ] 改动表每行都有规则编号；改动句复核 RISKY=0；
- [ ] `build` + `check` 零 FAIL；句 id 集合与顺序与优化前完全一致；
- [ ] **通过门：成文评审 REWRITE=0 且改动句复核 RISKY=0**。
