...
2. **句 id 冻结**：不增、不删、不重排、不改 id。确实需要拆句或并句时，停下来回到 ③ 按插句规则处理（字母后缀 `p2-37b`），并同步分镜——本阶段本身不做结构性增删。
...
5. **跨门约束**：系列纪律（不出现他集标题与序号词，[check_series.py](../scripts/check_series.py)）；改口播句可能新触发或解除画面文字复述门（[06](./06-storyboard.md)「画面文字不复述口播」）——改完必须重跑 `check`。
6. **下游代价知情**：已配音的集，改一句重配一句（逐句内容寻址缓存）；双语集改了主稿，`check --lang en` 会点名失配句 → 重译该句并过 [④C 译文保真](./04-verification.md)，或确认无需改动后 `build --lang en --accept <ids>`。
...
| 层 | 对应编辑层级 | 改什么 | 不改什么 |
|---|---|---|---|
| **L1 结构** | developmental | 篇章骨架、幕/节/段的顺序与分组、缺失的「转」、开头问题有没有兑现 | 句子措辞 |
...
9. **不许把一句话硬切成两行**：以逗号或顿号收尾、语义要到下一行才完整的句子，在**原有两个 id 之间重新分配文本**，让两行各自是完整的一句（例：「它能读懂你的需求，翻遍整个代码仓库，」＋「一次修改好几个文件……」→「它能读懂你的需求，翻遍整个代码仓库。」＋「它还能一次改好几个文件……」）。两行确实只够说一件事、无法各自成句时，属于并句，按第二节第 2 条回 ③ 处理。
10. **悬置只留给真悬念**：以冒号、破折号收尾、下一行才给答案的写法，只在真正需要停顿制造悬念的地方用，其余改成一句说完。
11. **幕尾钩子保留**：03 纪律要求每幕结尾留半句悬念钩到下一幕——这是结构性的，本阶段不删，只检查它和下一幕开头确实接得上。

**与配音分块的关系**：若采用按故事块合成的配音方式，块（≤3 句）是配音单位，beat（2–8 句）是阅读与画面单位，块嵌套在 beat 之内，二者不冲突；beat 边界也应是块边界。
...
1. **改前留底**：留底是第 4 步复核范围与 RISKY 回退原文的唯一来源，没有留底不得开始改稿。待改稿件在 ④ 清零时已 commit 的，以 `git diff HEAD -- <路径>` 定位改动、`git show HEAD:<路径>` 取回原文；否则一律改前复制到 `$P/.temp/prose-before/`，事后清理——新集稿件此时多半还未被 git 跟踪，`git diff` 看不到未跟踪文件，未提交的改动也会与 ③④ 的修改混在一起。不拿暂存区当留底：改稿中途再 `git add` 一次，底就被覆盖了。
...
4. **改动句复核**：以改前留底为基准，只对最终改动过的句子（含评审后的修正）回到 [④](./04-verification.md) 复核：A 节四级判定全做；B 节前三条（术语降落、指代距离、抽象连段）连同改动句的前后句一起看——话题链改主语、跨 id 搬运文本最容易在这三条上退步。门槛与 ④ 同口径：RISKY 与未处理 REWRITE 均须为零。判 RISKY 即回退原句；回退以改动表的行为单元，同一行的几个 id 整组回退原文——只回退其中一句，被搬运的文本会丢失或重复。回退句不再送评审（事实优先）；判 REWRITE 则按 ④ 修正后复核该句。未改动的句子不重复核查。
5. **重跑机器门**：`uv run --no-project $T/scripts/pipeline.py --project $P build`，然后 `check`（双语集会同时点名 en 失配句，按第二节第 6 条处理）。
...
- [ ] `build` + `check` 零 FAIL；句 id 集合与顺序与优化前完全一致；
