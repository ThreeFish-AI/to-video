...
4. **改动句复核**：以改前留底为基准，只对最终改动过的句子（含评审后的修正）重跑 [④A 真实性校验](./04-verification.md) 的四级判定，门槛与 ④ 同口径：RISKY 与未处理 REWRITE 均须为零。判 RISKY 即回退原句；回退以改动表的行为单元，同一行的几个 id 整组回退原文——只回退其中一句，被搬运的文本会丢失或重复。回退句不再送评审（事实优先）；判 REWRITE 则按 ④ 修正后复核该句。未改动的句子不重复核查。
