...
> **结论先行**：AI 稿件「断断续续、不像人写」的病灶在**篇章结构层**，不在词汇层。流水线在 ④ 双重校验之后新增正式阶段 **⑤ 成文优化**，按编辑行业的四层分工（结构 → 衔接 → 句子 → 词句）自顶向下改稿，只改表达、不改事实；改完交独立子代理做成文评审，改过的句子再回 ④ 复核兜底。规则正文在 [references/05-prose-refinement.md](../../references/05-prose-refinement.md)，本文只存设计依据（台账：[RSI-013](../.agents/issue.md)）。
...
| **A** | ④ 之后、分镜之前 | 优化对象是已校验的稿子；只对改动句回跑 ④（A 全做、B 前三条），复核成本与改动量成正比（编辑惯例：改过的段落重新核实）；优化后的 beat 分段直接喂给 ⑥ 切镜；④ 不动 | 分镜在 ⑥ 才产出，其成文规则需要回调 | **采纳**：06 规格验收加一条指针，规则仍只写在 05（单一事实源） |
...
- **④ 双重校验**：④ 判「对不对、懂不懂」，⑤ 判「顺不顺、像不像人」。④ B 节保留术语降落、指代距离、抽象连段等句级判据；篇章与节奏归 ⑤，两边不重复。⑤ 改过的句子回 ④ 复核时同样要过这三条——话题链改主语、跨 id 搬运文本最容易让它们退步。
...
- **配音故事块**（PR #17 已合入 main，story 档为新集默认配音，台账 RSI-011）：块（≤3 句 / ≤90 字）是配音单位，beat（2–8 句）是阅读与画面单位，块嵌套在 beat 之内，两者方向一致、不冲突。块起点由配音台本 `narration.cues.toml` 的 `[block.<id>]` 声明，⑤ 分好 beat 后复查块起点落在 beat 首句；台本 `[say]` 须去标点后与正文全等，⑤ 改字的句要同步台本（规则见 05 规格第二节第 6 条与第四节「与配音分块的关系」）。本条合并 main 时，PR #17 的 TTS 规格改动随改名识别落入 `references/07-tts-voice.md`。
