...
- **几何**：x4..1916（近贴屏幕左右边框，仅留一条竖线宽度呼吸——勿回缩与内容列对齐，全幅是章节条的产品形态）；段高 28（y14–42，圆角 4——勿胶囊化，播放头圆点是段上唯一圆形元素）、段间隙 8、段宽∝幕时长（含幕间
...
- **状态机 / 双色裁切**：已播部分 `text@0.9` 填充；段内文字跨亮填充与深轨两区，左右两层同位裁切——
  已填侧深字（`bg` 压亮填充）、未填侧亮字（当前章 `text`、未播章 `dim`），**文字色随播放头揭示翻转**
