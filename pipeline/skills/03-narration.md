...
- **形态**：`script/narration.en.md` 与主稿**句 id 1:1 对齐**——同 id、同序、同幕归属（幕标题译写）；`build --lang en` 派生 `narration.en.json` 并写**基线锁** `narration.en.lock.json`（翻译时主稿句文本的 digest 快照，gettext msgid 同构——主稿事后改稿，`check --lang en` 会点名失配句，译稿不会静默失鲜）。
...
- **系列纪律**：口播文本严禁出现其他集标题、集数序号或「上一集/上期/第 N 集」等顺序词——顺序只存在于视觉层与 `series.json`，据此保证发布顺序变更的 TTS 代价为零（「下期再见」一类顺序无关的收尾语不受此限；校验器 `$T/pipeline/scripts/check_series.py`）。
