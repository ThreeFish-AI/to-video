...
3. **风格定档？** 新集默认 **`story`（段落演绎，[VOICE-CLONING §4.5](../VOICE-CLONING.md)）**；写稿阶段应已同产 `script/narration.cues.toml`（skills/03 台本规约；无台本也能跑——自动分块 + 预设情绪，但块间情绪对比会打折）。换风格才需小样 A/B：`tts_sample.py --ref <样本> --style story --play`。**边听边记读错字**到 [PRON-GLOSSARY.md](../PRON-GLOSSARY.md)，用 `<字|读音>` 标注修（写稿规约见 [skills/03](./03-narration.md)）。**句尾英文词不赌采样、直接 CMU 标注**（采样层缺陷率约 50%；配方见 [VOICE-CLONING §5.4](../VOICE-CLONING.md) take 验收节）。听完 `--cleanup` 或 `pipeline.py clean-samples`（生物特征）。
...
2. 补写该集 `script/narration.cues.toml`（按 skills/03 台本规约通读全稿定块与情绪）；
...
- 长跑旁路监视：`uv run --no-project $T/pipeline/scripts/tts_progress.py --project $P`（按逐句 mp3 mtime 重建墙钟进度与热漂移告警，与合成进程零耦合——nohup 长跑时另开终端跑）
