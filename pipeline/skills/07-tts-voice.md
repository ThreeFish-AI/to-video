...
3. **风格定档？** 首次/换风格必过小样 A/B：`tts_sample.py --ref <样本> --all-styles --play`；再用**领航片段**（本集最难的 6–8 句：长分句/枚举清单/带小数点数字/含英文专名/含多音字）在该风格下整句合成试听。**边听边记读错字**到 [PRON-GLOSSARY.md](../PRON-GLOSSARY.md)，用 `<字|读音>` 标注修（写稿规约见 [skills/03](./03-narration.md)）。**句尾英文词不赌采样、直接 CMU 标注**（采样层缺陷率约 50%；配方见 [VOICE-CLONING §5.4](../VOICE-CLONING.md) take 验收节）。听完 `--cleanup` 或 `pipeline.py clean-samples`（生物特征）。
