...
| [scripts/check_playbook.py](./scripts/check_playbook.py) | 建模手册有界门（RSI 建模经验分支）：字数水位（CAP/HIGH/TARGET 滞回）+ 条目结构/权重/证锚点规则的唯一实现；零依赖、不需工作区 | `uv run --no-project $T/pipeline/scripts/check_playbook.py` |
...
- **对齐与失鲜**：`narration.en.md` 与主稿句 id 1:1（`build --lang en` 硬对齐门）；基线锁 `narration.en.lock.json` 记录翻译时的主稿句 digest，主稿改稿后 `check --lang en` 点名失配句。
