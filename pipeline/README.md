...
- 删掉软链，所有已部署分集与工作区的包装器都会立即报「找不到 to-video skill」；模板里的包装器沿用同一解析函数，今后新 scaffold 的分集与新 init 的工作区同样依赖它；
...
**安装形态**：请用 `git clone` 加软链安装，或设 `TO_VIDEO_HOME`。复制式安装（如 `npx skills add --copy`）若不保留软链，所有分集与工作区包装器（含新建的）都会失效。
