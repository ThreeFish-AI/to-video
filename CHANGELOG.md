...
- negentropy 侧自适配五步（本仓不提供迁移工程）：①61 个包装器探测行 `pipeline" / "scripts` → `scripts`；其中分集 frozen 包装器（tts / build_narration / qa_frames）须按 2.0.0 模板整文件覆盖——docstring 同批改过，只改探测行字节仍不等；②frozen 注释同步：本版修正了 7 个 frozen TS/TSX 的陈旧路径注释（`remotion.config.ts`、`i18n.tsx`、`NarrationAudio.tsx`、`Subtitle.tsx`、`SceneFade.tsx`、`ChapterProgress.tsx`、`motion/window.ts`，均为纯注释改动），md5 随之变化——当代集逐集整组拷齐后跑 `tsc --noEmit`，停在旧代的集只拷分代组外的文件、分代组按 ⑤ 登记（不做半同步）；漏掉此步则 `verify_skeleton --strict` 仍报 STALE，且 `render --lang en` 被骨架预检拦下（预检直比 NarrationAudio / Subtitle / ChapterProgress 字节、不查登记）；③工作区根补 `.to-video-root` 哨兵；④`mv ~/Library/Application\ Support/negentropy-influence/tts-store ~/Library/Application\ Support/to-video/tts-store`；⑤旧 skeleton.toml 中仍成立的 `[[drift]]` / `[[generation]]` 条目加 `skeleton.` 前缀搬入工作区 `to-video.toml`（已整组同步到模板的集不必搬）。
