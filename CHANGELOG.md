...
- negentropy 侧自适配四步（本仓不提供迁移工程）：①61 个包装器探测行 `pipeline" / "scripts` → `scripts`（与模板字节同步，verify_skeleton 随之对齐）；②工作区根补 `.to-video-root` 哨兵；③`mv ~/Library/Application\ Support/negentropy-influence/tts-store ~/Library/Application\ Support/to-video/tts-store`；④旧 skeleton.toml 中仍成立的 `[[drift]]` / `[[generation]]` 条目加 `skeleton.` 前缀搬入工作区 `to-video.toml`（已整组同步到模板的集不必搬）。
