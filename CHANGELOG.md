...
- 按 Agent Skills 规范重构（RSI-008）。**目录布局**：用 `git mv` 保留历史，迁移为 `pipeline/scripts` → `scripts/`、`pipeline/skills/NN-*.md` → `references/`（文件名、H1、⑥↔07 / ⑦↔06 错位都不变）、`pipeline/README.md` → `references/PIPELINE.md`、四本手册与 `stages.toml` → `references/`、`pipeline/templates` → `assets/`、`pipeline/tests` → `tests/`、`docs/quickstart` → `assets/quickstart/`。**兼容**：`pipeline/scripts → ../scripts` 软链是已部署 frozen 薄包装的 ABI，禁删；5 份包装器解析函数与全部 frozen 文件字节不变，已发布分集与工作区零改动可用；`pipeline/README.md` 改为迁移映射桩。**SKILL.md**：frontmatter 只用规范六字段（metadata 块式、allowed-tools 空格分隔、新增 compatibility；description 改写为「做什么 + Use when + 不用于」，去掉实现细节，补英文触发词）；正文重构为任务分流 → 工作流（显式修复重跑循环，去掉安装步）→ 九阶段速查 → 关键不变量（新增包装器 ABI）→ 运行时陷阱（Bash 调用间变量不保留、脚本当黑盒先 `--help`）→ 按需加载（文件 → 何时读）。env 表从 SKILL.md 迁入 `references/PIPELINE.md`「环境变量（机器属性注册处）」节，补登漏掉的 `INDEXTTS_SERVER`；PIPELINE.md 增加 `$T/` 布局树；PIPELINE / RSI / 01 / 06 / 08 各加一行目录；stages.toml 中 ⑦ 的 gate 补上「运动层铁律」；RSI.md 不变量 8、11 补执法指针，新增第 16 条（包装器 ABI），G3 纳入 evals 对拍；README 文档地图收敛为入口文档加知识索引指针。pytest 与 ruff 的路径同步为 `tests` 与 `^(scripts|tests)/`。
