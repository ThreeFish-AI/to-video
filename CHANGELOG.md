...
- `skeleton.toml` 历史登记清零：33 条 `[[drift]]`、2 组 `[[generation]]`（均为旧仓集名）与 `baselineOf` 全部移除；`.npmrc` 纯注释占位文件删除。**合法偏离登记面迁至工作区**（RSI-010）：`verify_skeleton.py` 改读 `$W/to-video.toml` 的 `[[skeleton.drift]]` / `[[skeleton.generation]]`（字段不变，表名加 `skeleton.` 前缀即可原样搬运；格式说明恢复在 skeleton.toml「合法偏离登记」「骨架分代」两节）；skill 侧 skeleton.toml 再出现登记键即大声退出——登记指向具体集，属内容，不随模板分发。
